...
```{r, echo=TRUE, eval=TRUE}
deceased_country_age_logit=glm(deceased~country*age+sex, data=d.corona, family=binomial)
...
For simplicity we write $\beta_0 + \beta_1x_{i1}  + \dots + \beta_7 x_{i7} = \mathbf{x_i}^\top \bfbeta$, where $\bfbeta = (\beta_0 \Hquad \cdots \Hquad \beta_7)^\top$ and $\mathbf{x_i} = (1\Hquad x_{i1} \Hquad \cdots \Hquad  x_{i7} )^\top$. Furthermore let $P(y_i = 1| \mathbf{X}={\mathbf{x}}_i) = p_i$. Starting from the expression for $p_i$ we get

\begin{align*}
 p_i &= \frac{e^{\mathbf{x_i}^\top \bfbeta}}{1+e^{\mathbf{x_i}^\top \bfbeta}} \\
 p_i + p_ie^{\mathbf{x_i}^\top \bfbeta} &= e^{\mathbf{x_i}^\top \bfbeta} \\
 p_i &= e^{\mathbf{x_i}^\top \bfbeta}(1-p_i) \\
 \frac{p_i}{1-p_i} &= e^{\mathbf{x_i}^\top \bfbeta}\\
 \log\left(\frac{p_i}{1-p_i}\right)  &= \mathbf{x_i}^\top \bfbeta = \beta_0 + \beta_1x_{i1}  + \dots + \beta_7 x_{i7}. \quad \blacksquare
\end{align*}
...
$\pi_k$ is the prior probability for class k, i.e $P(Y=k)$, so for the diabetes problem, $\pi_1$ is the probability for a random woman of having diabetes, while $\pi_0$ is the probability of not having diabetes. $\boldsymbol{\mu}_k$ is the mean vector for each class, so in this case $\mu_1$ and $\mu_0$ will be a vector of the mean values of each predictors for a correspondingly diabetic or non-diabetic person. $\boldsymbol{\Sigma}$ is the covariance matrix between the predictors and is in LDA assumed to be the same for all classes. $f_k(x)$ is the conditional distribution of the predictors of each class (diabetic or non diabetic), and is assumed to be normal distribution, with mean $\boldsymbol{\mu_k}$ and covariance matrix $\boldsymbol{\Sigma}$.
...
We observe that the LDA model has the highest AUC value, $0.849$, followed by the Logistic Regression model, AUC $= 0.8451$. Thus measured in the AUC metric, we would say that the LDA model performs the best. Since the difference in AUC value between Logistic Regression and LDA is relatively small, we would prefer the Logistic Regression model for interpretability.
...
A 95% confidence interval is therefore: [`r est_mean-1.96*rse`, `r est_mean+1.96*rse`].

### (iv)

We see that the fraction of variance explained by the model is estimated to be: `r summary(bodyfat_lm)$r.squared`. We have used bootstrapping to find the standard error to be `r rse`. The point estimate of $R^2$ is therefore uncertain. With 95% probability we have estimated that the fraction of variance explained by the model will lie in the interval [`r est_mean-1.96*rse`, `r est_mean+1.96*rse`].
